...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws  ServletException, IOException {
        ...
        if (header != null && header.startsWith("Bearer ")) {
            ...
            //token extract and validate and auth create and then security context ke andar set karunga.
                ...
                userRepository.findById(uuid).ifPresent(user -> {
                   //check for user not enabled
                    if(user.isEnable()){
                        ...
                        // final line : to set the auth to security context
